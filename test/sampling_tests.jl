...
const UnboundedTestCases = reduce(vcat, [
    [
        MonotonicTestData(
            name = "Normal (x>=0) $T",
...
const BoundedTestCases = reduce(vcat, [
    [
        MonotonicTestData(
...
const SymmetricTestCases = reduce(vcat, [
    [
...
function monotonic_sampling_tests(td::MonotonicTestData)
    @testset "N = $N, rng isa $(typeof(_rng)), array_generation = $array_generation" for N in [1, 2, 3, 4], _rng in [missing, Xoshiro(1234), MersenneTwister(1234)], array_generation in [true, false]
